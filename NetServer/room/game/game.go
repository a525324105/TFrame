...
const (
	...
	MaxGameFrame          uint32 = 30*60*3 + 1000000 //每局最大帧数
...
func (g *Game) JoinGame(id uint64, conn *network.Conn) bool {
	...
	p, ok := g.players[id]
	if !ok {
		logger.Error("[game(%d)] player[%d] join room failed", g.id, id)
		return false
	}
	...
	if k_Ready != g.State && k_Gaming != g.State {
		...
		logger.Error("[game(%d)] player[%d] game is over", g.id, id)
		return true
	}
	...
	if nil != p.client {
		...
		logger.Error("[game(%d)] player[%d] replace", g.id, id)
	}
...
func (g *Game) ProcessMsg(id uint64, msg *protocol.Packet) {
	...
	if !ok {
		logger.Error("[game(%d)] processMsg player[%d] msg=[%d]", g.id, player.id, msg.GetMessageID())
		return
	}
	...
	case pb.ID_MSG_Progress:
		...
		if err := msg.UnmarshalPB(m); nil != err {
			logger.Error("[game(%d)] processMsg player[%d] msg=[%d] UnmarshalPB error:[%s]", g.id, player.id, msg.GetMessageID(), err.Error())
			return
		}
	...
	case pb.ID_MSG_Ready:
		if k_Ready == g.State {
			...
			logger.Warn("[game(%d)] doReconnect [%d]", g.id, player.id)
		} else {
			logger.Error("[game(%d)] ID_MSG_Ready player[%d] state error:[%d]", g.id, player.id, g.State)
		}
	...
	case pb.ID_MSG_Input:
		...
		if !g.pushInput(player, m) {
			logger.Warn("[game(%d)] processMsg player[%d] msg=[%d] pushInput failed", g.id, player.id, msg.GetMessageID())
			break
		}
	...
	case pb.ID_MSG_Result:
		...
		logger.Info("[game(%d)] ID_MSG_Result player[%d] winner=[%d]", g.id, player.id, m.GetWinnerID())
		player.SendMessage(protocol.NewPacket(uint8(pb.ID_MSG_Result), nil))
	default:
		logger.Warn("[game(%d)] processMsg unknown message id[%d]", msgID)
	}
...
func (g *Game) Tick(now int64) bool {
	...
	case k_Ready:
		...
		if delta < MaxReadyTime {
			...
			if g.getOnlinePlayerCount() > 0 {
				...
				g.doStart()
				g.State = k_Gaming
				logger.Warn("[game(%d)] force start game because ready state is timeout ", g.id)
			} else {
				//全都没连进来，直接结束
				g.State = k_Over
				logger.Error("[game(%d)] game over!! nobody ready", g.id)
			}
		}
	...
	case k_Gaming:
		if g.checkOver() {
			g.State = k_Over
			logger.Info("[game(%d)] game over successfully!!", g.id)
			return true
		}

		if g.isTimeout() {
			g.State = k_Over
			logger.Warn("[game(%d)] game timeout", g.id)
			return true
		}
	...
	case k_Over:
		g.doGameOver()
		g.State = k_Stop
		logger.Info("[game(%d)] do game over", g.id)
		return true
	case k_Stop:
		return false
	}
...
func (g *Game) pushInput(p *Player, msg *pb.C2S_InputMsg) bool {
	...
	cmd := &pb.InputData{
		...
		X:          proto.Float32(msg.GetX()),
		Y:          proto.Float32(msg.GetY()),
		Z:          proto.Float32(msg.GetZ()),
