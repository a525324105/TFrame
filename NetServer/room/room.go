...
func (r *Room) OnJoinGame(id, pid uint64) {
	logger.Warn("[room(%d)] onJoinGame %d", id, pid)
}
func (r *Room) OnGameStart(id uint64) {
	logger.Warn("[room(%d)] onGameStart", id)
}

func (r *Room) OnLeaveGame(id, pid uint64) {
	logger.Warn("[room(%d)] onLeaveGame %d", id, pid)
}
func (r *Room) OnGameOver(id uint64) {
	atomic.StoreInt32(&r.closeFlag, 1)

	logger.Warn("[room(%d)] onGameOver", id)
...
func (r *Room) OnConnect(conn *network.Conn) bool {
	...
	r.inChan <- conn
	logger.Warn("[room(%d)] OnConnect %d", r.roomID, conn.GetExtraData().(uint64))
...
func (r *Room) OnMessage(conn *network.Conn, msg network.Packet) bool {
	...
	if !ok {
		logger.Error("[room] OnMessage error conn don't have id")
		return false
	}
...
func (r *Room) OnClose(conn *network.Conn) {
	...
	if id, ok := conn.GetExtraData().(uint64); ok {
		logger.Warn("[room(%d)] OnClose %d", r.roomID, id)
	} else {
		logger.Warn("[room(%d)] OnClose no id", r.roomID)
	}
...
func (r *Room) Run() {
	...
	defer func() {
		/*
			err := recover()
			if nil != err {
				logger.Error("[room(%d)] Run error:%+v", r.roomID, err)
			}*/
		r.game.Cleanup()
		logger.Warn("[room(%d)] quit! total time=[%d]", r.roomID, time.Now().Unix()-r.timeStamp)
	}()
	...
	logger.Info("[room(%d)] running...", r.roomID)

LOOP:
	for {
		select {
		case <-r.exitChan:
			logger.Error("[room(%d)] force exit", r.roomID)
			return
		case <-timeoutTimer.C:
			logger.Error("[room(%d)] time out", r.roomID)
			break LOOP
		case msg := <-r.msgQ:
			r.game.ProcessMsg(msg.id, msg.msg.(*protocol.Packet))
		case <-tickerTick.C:
			if !r.game.Tick(time.Now().Unix()) {
				logger.Info("[room(%d)] tick over", r.roomID)
				break LOOP
			}
		case c := <-r.inChan:
			...
			if ok {
				if r.game.JoinGame(id, c) {
					logger.Info("[room(%d)] player[%d] join room ok", r.roomID, id)
				} else {
					logger.Error("[room(%d)] player[%d] join room failed", r.roomID, id)
					c.Close()
				}
			} else {
				c.Close()
				logger.Error("[room(%d)] inChan don't have id", r.roomID)
			}
		...
		case c := <-r.outChan:
			if id, ok := c.GetExtraData().(uint64); ok {
				...
				c.Close()
				logger.Error("[room(%d)] outChan don't have id", r.roomID)
			}
		}
	}
	...
	for i := 3; i > 0; i-- {
		...
		logger.Info("[room(%d)] quiting %d...", r.roomID, i)
	}
}
